...
\section{Versuchsreihen und Vorgehensweise}
Es wurden mehrere Versuchsreihen auf dem Weg zum bereits beschriebenen Code unternommen,
die im Folgenden beschrieben werden.
\subsection{Versuch 1: Victm-DNS fragt direkt beim Attacker-DNS an}
Im ersten Versuch wurde der Victm-DNS als forwarding DNS Server konfiguriert, der anfragen direkt an den Attacker-DNS stellt. Ziel des Versuchs war es das richtige Antwort Format zu finden, damit der Victm-DNS die Antwort akzeptiert. Die Pakete wurden mit Wireshark analysiert und mit korrekten Paketen verglichen um Fehler zu finden.

\subsection{Versuch 2: Victm-DNS fragt beim Google-DNS Server an}
Beim zweiten Versuch wurde der Victm-DNS als forwarding DNS Server konfiguriert, der anfragen direkt an den Google-DNS Server stellt. Ziel war es herauszufinden ob man alle Pakete vor Eintreffen des echten Antwort Pakets senden kann. Zu diesen Zweck wurden die Pakete mit Hilfe von Wireshark auf dem Host und tshark auf dem Victm-DNS überwacht.

\subsection{Versuch 3: Victm-DNS als Caching-Nameserver}
Der dritte Versuch war Grundsätzlich gleich wie in Versuch 2 nur fragte der Victm-DNS nun bei den Rootnameservern an. Das Ziel war weiterhin herauszufinden ob man alle Pakete vor Eintreffen des echten Antwort Pakets senden kann.
